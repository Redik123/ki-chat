...
| `KI_PUBLIC_URL` | *(l'adresse par laquelle le client parle au serveur)* | Adresse publique (`https://hote:port`) pour les liens que suit un téléphone (QR code de l'atelier) et le lien des portes web. L'adresse réglée dans Admin → Serveur l'emporte |
...
- **Statistiques enrichies (0.1.40)** : la fiche se souvient des soixante derniers matchs et de cent points de RR ; les manches sont lues (ADR, KAST, premiers sangs, multi-kills, clutchs, poses/désamorçages) sans une requête de plus ; graphiques dessinés dans le client (courbe temporelle avec paliers, forme, agents, cartes, heures de jeu) ; MMR caché estimé d'après ses variations de RR. Toujours rien sur les neuf autres joueurs.
...
- **Porte web (0.1.44)** : Un membre qui peut créer des invitations ouvre une porte (`salon1`, 30 min à 6 h) depuis le bouton « Portes » de la barre latérale, et donne le lien — complet, `https://` compris, par exemple `https://ts.baws.fun:8080/invite` (ou `/s/invite`), avec son QR code — à quelqu'un qui n'a ni compte ni l'application. La page (servie par le serveur, sans dépendance, mobile d'abord) reprend la fenêtre de ki-chat en plus léger — la colonne des salons à gauche, le fil avec ses avatars et ses pseudos aux mêmes couleurs que dans l'application, la colonne « En ligne » à droite (les membres qui lisent le salon et les invités, tenue à jour par le serveur), le vocal et ses occupants sous le salon vocal ; sur un téléphone, une colonne et deux tiroirs. Elle demande un nom ; les membres qui peuvent expulser voient « Kevin veut rejoindre par le web », acceptent ou refusent en un clic ; et l'invité lit et écrit, marqué « (web) », dans un salon textuel temporaire — rien d'autre : ni la liste des salons, ni celle des membres. Un membre peut aussi l'amener dans **son** salon vocal : la voix passe par la même WebSocket (Opus via WebCodecs), le serveur chiffre et déchiffre à sa place. Le salon est effacé à la fermeture, dix minutes sans invité ou six heures au plus. L'adresse dont les liens sont faits se règle dans **Admin → Serveur → Adresse web publique** (`ts.baws.fun:8080` suffit, le `https://` est ajouté) ; sans elle, le client prend l'adresse par laquelle il joint le serveur, en HTTPS sur le port 8080. Tout est borné (cinq portes, vingt invités, une demande par adresse, limiteur, sas, budget d'écriture) et journalisé dans l'audit. « Lui offrir ki-chat » poste le lien de téléchargement dans le salon et pousse à la page de l'invité — à lui seul, jamais dans le salon lu par les autres invités — un code d'invitation à usage unique de sept jours. Voir [`PLAN-PORTE.md`](PLAN-PORTE.md).
...
- [x] **Glisser en vocal (0.1.48)** — Un occupant d'un salon vocal se glisse vers un autre salon et s'y retrouve (permission « Déplacer en vocal », rang au-dessus du sien ; se glisser soi-même, c'est y entrer). Et le clic droit → « Déplacer en vocal » ouvert depuis la barre des salons liste de nouveau les salons.
